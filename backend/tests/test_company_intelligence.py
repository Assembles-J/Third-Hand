...
def _stack(tmp_path):
    ...
    research_repo = ResearchDataRepository(store)
    gateway = ResearchDataGateway(research_repo)
    company_repo = CompanyIntelligenceRepository(store)
    ...
    return candidate_service, gateway, providers, service
...
def test_l3_company_context_is_deep_local_first_and_reuses_persisted_snapshots(tmp_path):
    candidate_service, gateway, providers, service = _stack(tmp_path)
    ...
    call_count_after_first = sum(calls.values())
    ...
    assert call_count_after_first == len(required_dataset_specs("L3"))
    assert sum(calls.values()) == call_count_after_first, "second build must reuse fresh local snapshots"
    assert second["research_ready"] is True
