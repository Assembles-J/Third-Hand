...
def test_legacy_xiaomi_lookup_is_promoted_to_local_directory_without_remote_call(tmp_path):
    store = make_store(tmp_path)
    ...
    for query in ("小米集团", "小米集团-W", "01810"):
...
def test_local_directory_supports_code_prefix(tmp_path):
    store = make_store(tmp_path)
    insert_cached_quote(store)
    repository = SymbolSearchRepository(store)

    results = repository.local_search("6005")

    assert [item["symbol"] for item in results] == ["600519"]
