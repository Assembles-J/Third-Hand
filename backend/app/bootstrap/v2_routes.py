...
def register_v2_routes(application) -> None:
    ...
    if not hasattr(application, "symbol_search_service_v2"):
        ...
        # Keep the existing /v1/market/symbols/resolve contract for Android and
        # OCR callers, but replace the blocking legacy implementation with the
        # cache-first v2 service. The legacy route resolves this module-global
        # function at request time, so no duplicate route is required.
