...
This adapter deliberately lives outside the legacy PortfolioStore monolith. It
reuses the store connection policy while building a small searchable directory
from securities the application has already seen, then persists remote lookup
...
class SymbolSearchRepository:
    ...
    def seed_from_existing_data(self) -> None:
        """Warm the directory from data already persisted by Third-Hand.

        Search should benefit from quotes, holdings, watchlists and legacy symbol
        lookups the user has already paid the cost to collect. No provider
        ...
        with self._seed_lock:
            ...
            with self.store._connect() as connection:
                ...
                # The legacy resolver already persisted successful directory
                # results here. Import those identities into the fast searchable
                # directory so upgrading the search path does not make the app
                # forget securities it already knows.
                try:
    ...
    def local_search(self, query: str, *, limit: int = 20) -> list[dict[str, str]]:
        ...
        with self.store._connect() as connection:
            rows = connection.execute(
                """
                SELECT market,symbol,name,currency,source,
                    CASE
    ...
    def cached_lookup(self, query: str) -> dict[str, object] | None:
        ...
        try:
            ...
            pass
        return {"matches": matches if isinstance(matches, list) else [], "updated_at": updated_at, "age_seconds": age_seconds}
