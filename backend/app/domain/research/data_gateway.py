...
def canonical_json(value: object) -> str:
    """Return strict canonical JSON for query and payload lineage.

    There is deliberately no ``default=str`` escape hatch. Provider adapters
    must normalize DataFrame, Decimal and custom objects before crossing this
    boundary, so opaque remote objects cannot silently enter an AI prompt.
    """
...
class ResearchDataRequest:
    ...
    def query_hash(self) -> str:
        # TTL, stale policy and coverage are consumption policy rather than query
        # identity. A larger follow-up request can therefore reuse the same local
        # dataset and fetch only the missing coverage.
