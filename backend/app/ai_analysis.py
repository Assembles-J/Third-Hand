...
class AiAnalysisService:
    ...
    def enrich(self, item: dict[str, object]) -> dict[str, object]:
        ...
        prompt = {"title": item["title"], "source": item["source_name"], "symbols": item["related_symbols"], "research_rules":self.store.research_rules(), "personal_review_cases":[{"lesson":case["lesson"],"outcome":case["outcome"],"position_band":case["position_band"],"planned_action":case["planned_action"],"confidence":case["confidence"],"evidence_links":json.loads(case["evidence_links"])} for case in cases]}
