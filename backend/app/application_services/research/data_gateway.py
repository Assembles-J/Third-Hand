...
class ResearchDataGateway:
    ...
    def _fresh(self, request: ResearchDataRequest, snapshot: ResearchDataSnapshot | None, *, now: datetime) -> bool:
        if snapshot is None or snapshot.schema_version != request.schema_version:
            return False
        if snapshot.usage_scope != "RESEARCH_ONLY" or self._missing_coverage(request, snapshot):
            return False
    ...
    def get_or_fetch(self, request: ResearchDataRequest, *, fetcher: ResearchFetcher) -> ResearchDataResult:
        ...
        try:
            ...
            # Persist-before-AI invariant: never return the transient provider
            # object. Reread the exact durable snapshot first.
            ...
            if fallback is not None:
                return ResearchDataResult(
                    replace(fallback, freshness_status="stale"),
