    ...
    for holding in holdings:
        ...
        if price is None:
            ...
            if technical_service:
                try:
                    technical = technical_service.assess(symbol)
                    evidence.append(f"趋势：{technical['trend']}；RSI {technical['rsi14']}；60日回撤 {technical['drawdown_60d_percent']}%")
                    confidence += 10
                    trace.append({"stage": "技术指标", "status": "ok", "detail": f"趋势 {technical['trend']}；RSI(14) {technical['rsi14']}；60 日回撤 {technical['drawdown_60d_percent']}%。"})
                    if technical["trend"] == "down" and technical["drawdown_60d_percent"] <= -12:
