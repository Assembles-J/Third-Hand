...
class PositionSizingEngine:
    ...
    def size(self, context: DecisionContext, action: str) -> PositionSizingResult:
        ...
        if action == "EXIT":
            ...
            return PositionSizingResult(status="ready", execution_disposition="ready", suggested_quantity=sellable_quantity, target_quantity=current_quantity - sellable_quantity, target_position_percent=0.0 if sellable_quantity >= current_quantity else ((current_quantity - sellable_quantity) * (position.current_price or 0) / assets * 100 if assets else None), **common)
        missing = []
        ...
        if action == "REDUCE":
            ...
            return PositionSizingResult(status="ready", execution_disposition="ready", suggested_quantity=suggested, target_quantity=current_quantity - suggested, target_position_percent=round((current_quantity - suggested) * entry / assets * 100, 4), quantity_by_position_cap=target_quantity, lot_size=lot, entry_price=entry, **common)
