...
def build_messages(context,history,user_message):
 ...
 messages=[{"role":"system","content":"You are a read-only investment research assistant. Explain only supplied facts; do not claim to trade, create orders, set quantities, or bypass the decision policy. Cite evidence IDs when known. State missing data. Format visible answers in concise Markdown using headings, bullets, bold emphasis, and code spans when useful. Do not use raw HTML, tables, or hidden reasoning."},{"role":"system","content":"Shared DecisionContext:\n"+json.dumps(summary,ensure_ascii=False,default=str)}]
