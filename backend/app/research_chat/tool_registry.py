"""Explicit read-only/proposal-only tools exposed to the research model.

Research Chat may inspect persisted data and propose user-confirmed data changes.
It cannot execute paper trades, alter ActionPolicy, change formal sizing, or
bypass the historical DecisionReport -> next eligible quote execution boundary.
"""
...
def _parameters(name: str) -> dict[str, object]:
    ...
    if name == "propose_data_change":
        return {
            "type": "object",
            "properties": {
                "entity": {"type": "string", "enum": ["holding", "trade_plan", "personal_rule"]},
                "operation": {"type": "string", "enum": ["create", "update", "delete"]},
                "summary": {"type": "string", "maxLength": 500},
            },
            "required": ["entity", "operation", "summary"],
            "additionalProperties": False,
        }
