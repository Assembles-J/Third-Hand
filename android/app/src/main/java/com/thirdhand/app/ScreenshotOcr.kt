...
object ScreenshotOcr {
    suspend fun scan(context: Context, imageUri: Uri): List<RecognizedHolding> {
        ...
        return lines.mapIndexedNotNull { index, nameLine ->
            val box = nameLine.boundingBox ?: return@mapIndexedNotNull null
            val name = nameLine.text.trim()
            if (box.left.toFloat() > imageWidth * 0.32f || !name.contains(Regex("[\\u4e00-\\u9fff]"))) return@mapIndexedNotNull null
            ...
            if (quantity != null && quantity > 0 && cost != null && cost >= 0) {
                RecognizedHolding("row-${box.top}-${box.left}-$index", name, quantity, cost)
            } else null
        }.distinctBy { it.clientRowId }
    }
