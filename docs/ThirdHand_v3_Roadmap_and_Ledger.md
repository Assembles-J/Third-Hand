...
- **Audit / observability:** `执行链路记录`, recent executed fills and per-fill
  `分析记录` stay first-class. The Android click path now reloads the existing
  paper decision-audit DTO and optional decision lineage before opening the audit
  dialog, preserving the intended existing drill-down rather than adding a new
  authority path.
- **Language cleanup:** the page title is `策略执行` with `模拟账套 · 决策驱动 · 风控执行`;
  legacy English section subtitles and the ambiguous `影子交易` label are removed.
  Empty state wording no longer claims the app will “寻找机会”.
- **Backend/API:** unchanged. Existing paper dashboard/config/run/run-detail/
  decision-audit contracts are reused exactly; no new DTO, endpoint, provider,
  performance statistic or manual order-entry action is introduced.
- **Safety boundary:** this is the existing Formal-decision simulated-account
...
- **Screenshot coverage:** a representative 420dp compact `策略执行` ready state is
  added for Compose rendering. Its approved hash must be visually reviewed and
  locked before repository acceptance.
- **Delivery status:** `ANDROID_IMPLEMENTED / CI_SCREENSHOT_DEVICE_ACCEPTANCE_PENDING`.
  Android unit/compile, screenshot render/hash, Debug/Release, repository
  governance/ci-gate and a physical-device `策略` walkthrough remain required
