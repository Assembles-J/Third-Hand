...
| Evaluation | N3.1-N3.6 are `BACKEND_READY` stacked slices; Android/acceptance pending | Immutable experiment identity is bound to an exact `ExperimentUniverseSnapshot`; OutcomeResolver, StrategyEvaluation and BenchmarkPolicy enforce frozen membership and point-in-time market history. N3.6 adds a GET-only `/v1/lab` API over already-persisted experiment, outcome, strategy-evaluation and benchmark snapshots. The HTTP layer never refreshes providers, resolves fresh outcomes or mutates trading state; unavailable portfolio metrics and non-materialized PENDING counts remain explicit rather than being coerced to zero. No Lab route may read mutable Personal Universe membership dynamically. |
