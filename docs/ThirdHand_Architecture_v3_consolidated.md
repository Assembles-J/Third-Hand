...
| Personal universe / review cadence | PUX1 Android-visible; PUX2 domain/persistence foundation implemented; PUX3 designed | PUX1 has a first-class Android Watchlist and remains device-acceptance pending. PUX2 now has versioned deterministic ReviewPlan/AnalysisBudget contracts and append-only persistence; scheduler/API/Android consumption remains pending under #93. Bounded Discovery (#94) remains designed. Universe membership, review permission, analysis depth and trading authority remain separate policy owners. |
...
The first PUX2 implementation slice uses policy version
`PUX2_REVIEW_V1`. It persists append-only `ReviewPlan` decisions containing
mode, permitted analysis depth, stable reason codes, last/next review,
routine-budget availability and any explicit override. Routine full research is
limited to once per symbol per Beijing-local day. Only `material_change` and an
explicit user request may override that routine budget, and the override reason
is persisted. Hard guard obligations remain `GUARD_ONLY`; they do not silently
grant full research or a trading action. Scheduler consumption and Android/API
visibility are later #93 slices and this foundation alone is not `PRODUCT_DONE`.
