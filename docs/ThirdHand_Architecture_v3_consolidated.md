...
| Evaluation | N3.1-N3.4 are `BACKEND_READY` stacked slices; later N3 slices pending | Immutable experiment identity is bound to an exact `ExperimentUniverseSnapshot`; OutcomeResolver rejects sources outside it, and StrategyEvaluation independently revalidates aggregate decision/episode membership and binds the universe hash into each evaluation snapshot. Mutable Personal Watchlist/positions/Discovery cannot alter experiment samples. Portfolio total return/max drawdown/turnover remain unavailable until experiment-level equity-curve truth exists. |
