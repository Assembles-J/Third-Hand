...
| Evaluation | N3.1-N3.3 are `BACKEND_READY` stacked slices; later N3 slices pending | Immutable experiment identity is bound to an exact `ExperimentUniverseSnapshot`; outcome contracts define what may be measured; the local-only OutcomeResolver now rejects any decision/episode outside that frozen membership before resolving market/execution facts. Mutable Personal Watchlist/positions/Discovery cannot alter experiment samples, and Evaluation still has no provider refresh or production trading write path. |
