...
| Personal universe / review cadence | PUX1 backend/API implemented; Android first-class Watchlist pending; PUX2/PUX3 designed | PUX1 backend/API is accepted via #86 with typed Personal Universe, additive Watchlist metadata and local-only composition. Android first-class Watchlist is tracked by #92. ReviewPolicy/AnalysisBudget (#93) and bounded Discovery (#94) remain designed follow-ups. Universe membership, review permission, analysis depth and trading authority remain separate policy owners. |
