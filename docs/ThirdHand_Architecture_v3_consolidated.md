...
| Evaluation | N3.1-N3.2 are `BACKEND_READY` stacked slices; later N3 slices pending | Immutable experiment identity is bound to an exact `ExperimentUniverseSnapshot`; immutable Decision/Execution/TradeEpisode outcome contracts and versioned OutcomePolicy now define what may be measured. Mutable Personal Watchlist/positions/Discovery cannot alter experiment samples, and Evaluation still has no provider refresh or production trading write path. |
