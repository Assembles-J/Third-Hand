...
| Evaluation | N3.1-N3.5 are `BACKEND_READY` stacked slices; Lab API/Android/acceptance pending | Immutable experiment identity is bound to an exact `ExperimentUniverseSnapshot`; OutcomeResolver and StrategyEvaluation enforce that frozen membership. BenchmarkPolicy now provides explicit market-aware decision-window baselines from local point-in-time daily history: explicit index/symbol policies require a declared market/symbol, while equal-weight baselines use only same-market members from the frozen experiment universe and fail closed on incomplete constituents. No benchmark may read the mutable Personal Universe dynamically. Portfolio total/excess benchmark return remains unavailable until aligned experiment/benchmark equity curves exist. |
